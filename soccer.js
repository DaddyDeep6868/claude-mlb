 ...
 * and the orange/teal/gold palette. Layered over the compiled dc app
 * without touching the template.
 *
 * Markets: Anytime Goalscorer (home-run analog), Match Result (1X2),
 * Over/Under 2.5 Goals, Both Teams To Score, and Corners. The fixture
 * slate is anchored to the current date so there is always a live Today
 * slate with kickoff times and live/FT status.
 */
(function () {
  'use strict';
  ...
  function statusOf(d) {
    ...
    if (now < k + LIVE_MIN * 60000) { var el = Math.round((now - k) / 60000); return { key: 'live', label: 'LIVE ' + (el > 90 ? '90+' : el) + "'", rel: 'in play', color: NEG, dot: NEG }; }
  ...
  var PLAYERS = [
    ...
    { n: 'Kylian Mbappe', t: 'FRA', pos: 'FW', threat: 0.78, min: 0.94 }, { n: 'Ousmane Dembele', t: 'FRA', pos: 'FW', threat: 0.48, min: 0.82 },
    ...
    { n: 'Cody Gakpo', t: 'NED', pos: 'FW', threat: 0.48, min: 0.85 }, { n: 'Memphis Depay', t: 'NED', pos: 'FW', threat: 0.46, min: 0.72 },
    { n: 'Jamal Musiala', t: 'GER', pos: 'MF', threat: 0.50, min: 0.90 }, { n: 'Kai Havertz', t: 'GER', pos: 'FW', threat: 0.46, min: 0.84 }, { n: 'Florian Wirtz', t: 'GER', pos: 'MF', threat: 0.44, min: 0.86 },
    { n: 'Christian Pulisic', t: 'USA', pos: 'FW', threat: 0.42, min: 0.90 }, { n: 'Folarin Balogun', t: 'USA', pos: 'FW', threat: 0.38, min: 0.80 },
    { n: 'Santiago Gimenez', t: 'MEX', pos: 'FW', threat: 0.40, min: 0.82 }, { n: 'Jonathan David', t: 'CAN', pos: 'FW', threat: 0.42, min: 0.88 }, { n: 'Alphonso Davies', t: 'CAN', pos: 'DF', threat: 0.26, min: 0.90 },
    { n: 'Darwin Nunez', t: 'URU', pos: 'FW', threat: 0.48, min: 0.85 }, { n: 'Federico Valverde', t: 'URU', pos: 'MF', threat: 0.36, min: 0.92 }, { n: 'Andrej Kramaric', t: 'CRO', pos: 'FW', threat: 0.38, min: 0.78 },
    { n: 'Romelu Lukaku', t: 'BEL', pos: 'FW', threat: 0.50, min: 0.85 }, { n: 'Kevin De Bruyne', t: 'BEL', pos: 'MF', threat: 0.42, min: 0.86 }, { n: 'Youssef En-Nesyri', t: 'MAR', pos: 'FW', threat: 0.40, min: 0.84 }, { n: 'Kaoru Mitoma', t: 'JPN', pos: 'FW', threat: 0.36, min: 0.86 }
  ];
  ...
  function buildMatches() { var now = new Date(); return PAIRS.map(function (p, i) { var pl = PLAN[i]; return { h: p[0], a: p[1], date: new Date(now.getFullYear(), now.getMonth(), now.getDate() + pl.o, pl.h, 0, 0, 0), stage: 'Round of 16', venue: VENUES[i] }; }); }
  ...
  function goalscorers() {
    ...
    PLAYERS.forEach(function (p) {
      var fx = opp[p.t]; if (!fx) return; var team = TEAMS[p.t], op = TEAMS[fx.opp];
      ...
      out.push({ name: p.n, team: p.t, teamName: team.name, flag: team.flag, pos: p.pos, opp: fx.opp, oppName: op.name, home: fx.home, date: fx.m.date, stage: fx.m.stage, prob: prob, am: mk.am, edge: mk.edge, ev: mk.ev, score: mk.score, init: p.n.split(' ').map(function (w) { return w[0]; }).slice(-2).join('') });
    });
  ...
  function marketsForMatch(m) {
    ...
    return [
      { grp: 'Result', label: H.name + ' win', prob: mm.pH, key: '1x2h' + m.h }, { grp: 'Result', label: 'Draw', prob: mm.pD, key: '1x2d' + m.h }, { grp: 'Result', label: A.name + ' win', prob: mm.pA, key: '1x2a' + m.h },
      { grp: 'Goals', label: 'Over 2.5 goals', prob: mm.over25, key: 'o25' + m.h }, { grp: 'Goals', label: 'Under 2.5 goals', prob: mm.under25, key: 'u25' + m.h }, { grp: 'Goals', label: 'Both teams to score', prob: mm.btts, key: 'btts' + m.h },
      { grp: 'Corners', label: 'Over 9.5 corners', prob: mm.over95c, key: 'c9o' + m.h }, { grp: 'Corners', label: 'Under 9.5 corners', prob: mm.under95c, key: 'c9u' + m.h }
    ].map(function (s) { return Object.assign(s, market(s.prob, s.key), { match: lbl, date: m.date }); });
  }
  ...
  function crest(code) { var t = TEAMS[code]; return '<div style="width:40px;height:40px;flex:none;border-radius:50%;background:' + INSET + ';display:grid;place-items:center;font-size:20px;border:1px solid rgba(255,255,255,.08)">' + t.flag + '</div>'; }
  ...
  function matchCardFull(m, opts) {
    ...
    var head = '<div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:14px"><div style="display:flex;align-items:center;gap:11px;min-width:0">' + crest(m.h)
      ...
      + '<div style="font-size:11.5px;color:' + MUT + ';margin-top:3px">' + esc(m.stage) + ' \u00B7 ' + dayLabel(m.date) + ' ' + fmtTime(m.date) + ' \u00B7 ' + esc(m.venue) + '</div></div>' + crest(m.a) + '</div>'
    ...
    if (opts.corners) {
      ...
      body = '<div style="display:flex;gap:12px;margin-bottom:13px">' + probBar(H.name, mm.pH, AC) + probBar('Draw', mm.pD, MUT3) + probBar(A.name, mm.pA, POS) + '</div>'
        + '<div style="display:flex;flex-direction:column;gap:7px">' + oddRow(H.name + ' to win', mm.pH, '1x2h' + m.h) + oddRow('Draw', mm.pD, '1x2d' + m.h) + oddRow(A.name + ' to win', mm.pA, '1x2a' + m.h)
  ...
  function miniMatch(m) {
    ...
    return '<button class="dlcard dlbtn" data-act="tab" data-tab="matches" style="text-align:left;width:100%;background:' + CARD + ';border:1px solid ' + LINE + ';border-radius:14px;padding:13px 14px;display:flex;flex-direction:column;gap:9px;cursor:pointer;color:' + TXT + '">'
      ...
      + '<div style="display:flex;align-items:center;justify-content:space-between;padding-top:8px;border-top:1px solid ' + LINE + '"><span style="font-size:11.5px;color:' + MUT + '">Model pick</span>'
  ...
  function lockCard(p, rank) {
    return '<div class="dlcard" style="position:relative;background:' + INSET2 + ';border:1px solid rgba(255,255,255,.09);border-radius:14px;padding:14px;display:flex;flex-direction:column;gap:11px;flex:1 1 180px;min-width:0;overflow:hidden">'
      ...
      + '<div style="font-size:11.5px;color:' + MUT2 + ';line-height:1.45">Model ' + pct(p.prob) + ' vs book \u2014 edge ' + (p.edge >= 0 ? '+' : '') + (p.edge * 100).toFixed(1) + 'pts, EV ' + (p.ev > 0 ? '+' : '') + p.ev.toFixed(1) + '/$100.</div></div>';
  }
  function playerCard(p) {
    return '<div class="dlcard" style="background:' + CARD + ';border:1px solid ' + LINE + ';border-radius:16px;padding:15px;display:flex;flex-direction:column;gap:12px">'
      ...
      + '<div style="display:flex;gap:8px">' + miniStat('Goal \u00B7 model', pct(p.prob), TXT) + miniStat('Anytime', fmtAm(p.am), scoreColor(p.score)) + miniStat('EV /$100', (p.ev > 0 ? '+' : '') + p.ev.toFixed(1), evColor(p.ev)) + '</div></div>';
  }
  ...
  function renderToday() {
    ...
    var stats = '<div class="dlstats" style="display:grid;grid-template-columns:repeat(5,1fr);gap:12px;margin-bottom:26px">'
      ...
      + statCard('Value bets', value.length, POS, 'model edges')
    ...
    var gsHtml = '<div class="dlgrid" style="display:grid;grid-template-columns:repeat(auto-fill,minmax(280px,1fr));gap:12px">' + gsBoard.map(playerCard).join('') + '</div>';
    var dateStr = now.toLocaleDateString(undefined, { weekday: 'long', month: 'long', day: 'numeric' });

    return pageHead('COMMAND CENTER', 'Today\u2019s best goals', esc(dateStr) + ' \u00B7 World Cup 2026 knockouts. Model-priced anytime goalscorer, match result, goals and corner edges.')
      + stats + locksBanner
      ...
      + sectionHead(GOLD, 'Top Goalscorers', 'model vs book', '<button class="dlbtn" data-act="tab" data-tab="radar" style="background:none;border:1px solid rgba(255,255,255,.12);color:' + MUT + ';font-family:' + FB + ';font-weight:600;font-size:12px;padding:6px 12px;border-radius:9px;cursor:pointer">Full radar \u2192</button>') + gsHtml;
  }
  function dateChips() {
  ...
  function renderMatches(corners) {
    ...
    var html = pageHead(corners ? 'SET-PIECE LAB' : 'MATCH CENTER', corners ? 'Corner edges' : 'Match markets', corners ? 'Projected corner volume and over/under lines across the knockout slate.' : 'Result (1X2), over/under 2.5 goals and both-teams-to-score, model-priced per fixture.') + dateChips();
  ...
  function renderRadar() {
    ...
    return pageHead('GOALSCORER RADAR', 'Anytime goalscorer edges', 'Every attacker priced by threat, matchup and minutes \u2014 the home-run analog for soccer.')
      + strip + '<div style="display:flex;gap:8px;flex-wrap:wrap;margin:4px 0 16px">' + chips + '</div>'
      + '<div class="dlgrid" style="display:grid;grid-template-columns:repeat(auto-fill,minmax(280px,1fr));gap:12px">' + list.map(playerCard).join('') + '</div>';
  }
  ...
  function render() {
    ...
    var liveCount = MATCHES.filter(function (m) { return statusOf(m.date).key === 'live'; }).length;
    ...
    var header = '<header style="position:sticky;top:0;z-index:40;display:flex;align-items:center;gap:16px;padding:14px 22px;background:rgba(10,12,17,.8);backdrop-filter:blur(14px);border-bottom:1px solid ' + LINE + '">'
      ...
      + '<div class="dlhide" style="display:flex;align-items:center;gap:8px;padding:6px 12px;border-radius:10px;background:' + (liveCount ? 'rgba(255,107,107,.1)' : 'rgba(53,208,192,.08)') + ';border:1px solid ' + (liveCount ? 'rgba(255,107,107,.3)' : 'rgba(53,208,192,.22)') + '"><span class="dlpulse" style="width:7px;height:7px;border-radius:50%;background:' + (liveCount ? NEG : POS) + '"></span><span style="font-size:12.5px;font-weight:600;color:' + (liveCount ? NEG : POS) + '">' + (liveCount ? liveCount + ' live now' : 'Model \u00B7 sample slate') + '</span></div>'
    ...
    var main = '<main style="padding:22px 26px 60px;min-width:0;overflow:auto">' + body()
      + '<div style="margin-top:30px;padding-top:16px;border-top:1px solid ' + LINE + ';font-size:11.5px;color:' + MUT + ';line-height:1.6">Model probabilities are computed from team strength & player threat on an offline sample slate anchored to today. Live odds wire through the OddsBlaze <code>league=' + esc(LEAGUE) + '</code> endpoint when the server is connected. For entertainment only.</div></main>';
